            <!-- ========== Left Sidebar Start ========== -->
            <div class="left side-menu">

                <!-- LOGO -->
                <div class="topbar-left">
                    <div class="">
                        <a href="/admin" class="logo text-center">MAROON</a>
                        <!--<a href="index.html" class="logo"><img src="assets/images/logo-sm.png" height="36" alt="logo"></a>-->
                    </div>
                </div>

                <div class="sidebar-inner slimscrollleft">
                    <div id="sidebar-menu">
                        <ul>
                            <!--Main-->
                            <li class="menu-title">Main</li>
                                <!--Dashboard-->
                                <li>
                                    <a href="/admin" class="waves-effect"><i class="mdi  mdi-view-dashboard"></i><span>Dashboard</span></a>
                                </li>
                            <!--Ecommerce Menu--->
                            <li class="menu-title">Ecommerce</li>
                                <!--Media-->
{{--                                <li class="">--}}
{{--                                    <a href="/admin/media" class="waves-effect">--}}
{{--                                        <i class="mdi mdi-package-variant"></i>--}}
{{--                                        <span> Media --}}
{{--                                            <span class="pull-right">--}}
{{--                                                <i class="mdi mdi-chevron-right"></i>--}}
{{--                                            </span> --}}
{{--                                        </span>--}}
{{--                                    </a>--}}
{{--                                </li>--}}
                            <!--Slider-->
                            <li class="">
                                <a href="/admin/slide" class="waves-effect">
                                    <i class="mdi mdi-package-variant"></i>
                                    <span> Slider
                                            <span class="pull-right">
                                                <i class="mdi mdi-chevron-right"></i>
                                            </span>
                                        </span>
                                </a>
                            </li>

                            <li class="">
                                <a href="/admin/category" class="waves-effect">
                                    <i class="mdi mdi-package-variant"></i>
                                    <span> Category
                                            <span class="pull-right">
                                                <i class="mdi mdi-chevron-right"></i>
                                            </span>
                                        </span>
                                </a>
                            </li>



                                <!--Products-->
                                <li class="has_sub">
                                    <a href="" class="waves-effect">
                                        <i class="mdi mdi-package-variant"></i>
                                        <span> Products 
                                            <span class="pull-right">
                                                <i class="mdi mdi-chevron-right"></i>
                                            </span> 
                                        </span>
                                    </a>
                                    <ul class="list-unstyled">
                                        <li><a href="/admin/product/create">Add New Product</a></li>
                                        <li><a href="/admin/product">Product List</a></li>
                                    </ul>
                                </li>
                                <!--Orders-->
                                <li class>
                                    <a href="/admin/orders" class="waves-effect">
                                        <i class="mdi mdi-cube-outline"></i> 
                                        <span> 
                                            Orders
                                            <span class="badge badge-pill badge-primary pull-right">
                                            20+
                                            </span>
                                        </span> 
                                    </a>
{{--                                    <ul class="list-unstyled">--}}
{{--                                        <li><a href="/admin/orders">Orders</a></li>--}}
{{--                                        <li><a href="/admin/orders-history">Orders History</a></li>--}}
{{--                                    </ul>--}}
                                </li>
                                <!--Customers-->
                                <li class>
                                    <a href="/admin/customer-list" class="waves-effect">
                                        <i class="mdi mdi-account"></i> 
                                        <span> 
                                            Customers
                                            <span class="badge badge-pill badge-primary pull-right">
                                            20+
                                            </span>
                                        </span> 
                                    </a>
{{--                                    <ul class="list-unstyled">--}}
{{--                                        <li><a href="/admin/customer-list">Customer List</a></li>--}}
{{--                                        <li><a href="">Send SMS</a></li>--}}
{{--                                    </ul>--}}
                                </li>
                                <!--Settings-->
                                <li class="has_sub">
                                    <a href="" class="waves-effect">
                                        <i class="mdi mdi-settings"></i>
                                        <span> Shop Settings
                                            <span class="pull-right">
                                                <i class="mdi mdi-chevron-right"></i>
                                            </span> 
                                        </span>
                                    </a>
                                    <ul class="list-unstyled">
                                        <li>
                                            <a href="/admin/coupon" class="waves-effect">
                                                <i class="mdi mdi-tag-multiple"></i>
                                                <span> Coupons
                                                    <span class="pull-right">
                                                        <i class="mdi mdi-chevron-right"></i>
                                                    </span> 
                                                </span>
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                            <!--Reporting Panel-->
{{--                            <li class="menu-title">Reports</li>--}}
{{--                                <!--Report 1-->--}}
{{--                                <li class="has_sub">--}}
{{--                                    <a href="" class="waves-effect">--}}
{{--                                        <i class="mdi mdi-email-outline"></i>--}}
{{--                                        <span> Report 2--}}
{{--                                            <span class="pull-right">--}}
{{--                                                <i class="mdi mdi-chevron-right"></i>--}}
{{--                                            </span> --}}
{{--                                        </span>--}}
{{--                                    </a>--}}
{{--                                    <ul class="list-unstyled">--}}
{{--                                        <li><a href="email-inbox.html">Inbox</a></li>--}}
{{--                                    </ul>--}}
{{--                                </li>--}}
                            <!--Configurations Panel-->
                            <li class="menu-title">Configurations</li>
                                <!--Pages-->
                                <li class="has_sub">
                                    <a href="" class="waves-effect">
                                        <i class="mdi mdi-file-document"></i>
                                        <span> Pages
                                            <span class="pull-right">
                                                <i class="mdi mdi-chevron-right"></i>
                                            </span> 
                                        </span>
                                    </a>
                                    <ul class="list-unstyled">
                                        <li>
                                            <a href="/admin/disclaimer" class="waves-effect">
                                                <i class="mdi mdi-truck-delivery"></i>
                                                <span>Disclaimer</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="/admin/privacy-policy" class="waves-effect">
                                                <i class="mdi mdi-truck-delivery"></i>
                                                <span>Privacy Policy</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="/admin/payment-policy" class="waves-effect">
                                                <i class="mdi mdi-truck-delivery"></i>
                                                <span>Payment Policy</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="/admin/refund-policy" class="waves-effect">
                                                <i class="mdi mdi-truck-delivery"></i>
                                                <span>Refund Policy</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="/admin/return-policy" class="waves-effect">
                                                <i class="mdi mdi-truck-delivery"></i>
                                                <span>Return Policy</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="/admin/damaged-lost-policy" class="waves-effect">
                                                <i class="mdi mdi-truck-delivery"></i>
                                                <span>Damaged/Lost Policy</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="/admin/store-location" class="waves-effect">
                                                <i class="mdi mdi-map"></i>
                                                <span>Store Location</span>
                                            </a>
                                        </li>
                                        <li>
                                            <a href="/admin/about-us" class="waves-effect">
                                                <i class="mdi mdi-map"></i>
                                                <span>About Us</span>
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                                <!--Users-->
                                <li class="has_sub">
                                    <a href="" class="waves-effect">
                                        <i class=" mdi mdi-account-multiple"></i>
                                        <span> Users
                                            <span class="pull-right">
                                                <i class="mdi mdi-chevron-right"></i>
                                            </span> 
                                        </span>
                                    </a>
                                    <ul class="list-unstyled">
                                        <li><a href="/admin/user-list">User List</a></li>
{{--                                        <li><a href="">SMS</a></li>--}}
                                    </ul>
                                </li>
                                <!--Settings-->
{{--                                <li class="has_sub">--}}
{{--                                    <a href="" class="waves-effect">--}}
{{--                                        <i class="mdi mdi-settings"></i>--}}
{{--                                        <span> Settings--}}
{{--                                            <span class="pull-right">--}}
{{--                                                <i class="mdi mdi-chevron-right"></i>--}}
{{--                                            </span> --}}
{{--                                        </span>--}}
{{--                                    </a>--}}
{{--                                    <ul class="list-unstyled">--}}
{{--                                        <li><a href="">Inbox</a></li>--}}
{{--                                        <li><a href="">Email Read</a></li>--}}
{{--                                        <li><a href="">Email Compose</a></li>--}}
{{--                                    </ul>--}}
{{--                                </li>--}}
                        </ul>
                    </div>
                    <div class="clearfix"></div>
                </div> <!-- end sidebarinner -->
            </div>
            <!-- Left Sidebar End -->